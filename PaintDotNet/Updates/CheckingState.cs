namespace PaintDotNet.Updates
{
    ...
    internal class CheckingState : UpdatesState
    {
        ...
        private const string versionManifestTestRelativeUrl = "/updates/versions.7.test.txt";
        ...
        private static void CheckForUpdates(out PdnVersionManifest manifestResult, out int latestVersionIndexResult, out Exception exception)
        {
            ...
            if (updatesManifest != null)
            {
                ...
                bool flag = "1" == Settings.SystemWide.GetString("CHECKFORBETAS", "0");
                int index = latestStableVersionIndex;
                if ((flag && (latestBetaVersionIndex != -1)) && ((latestStableVersionIndex == -1) || (updatesManifest.VersionInfos[latestBetaVersionIndex].Version >= updatesManifest.VersionInfos[latestStableVersionIndex].Version)))
                {
        ...
        private static PdnVersionManifest GetUpdatesManifest(out Exception exception)
        {
            try
            {
                ...
                byte[] bytes = Utility.DownloadSmallFile(uri);
        ...
        public override void OnEnteredState()
        {
            ...
            ThreadPool.QueueUserWorkItem(new WaitCallback(this.DoCheckThreadProc));
        ...
        public override void ProcessInput(object input, out PaintDotNet.State newState)
        {
            ...
            else if (input.Equals(PrivateInput.GoToError))
            {
                ...
                {
                    str = Utility.WebExceptionToErrorMessage((WebException) this.exception);
                }
                else
                {
                    str = PdnResources.GetString2("Updates.CheckingState.GenericError");
                }
        ...
        {
            string[] strArray = versions.Split(new char[] { ',' });
            if ((strArray.Length == 0) || ((strArray.Length == 1) && (strArray[0].Length == 0)))
            {
                return ArrayUtil.Empty<Version>();
            }
        ...
        private static string VersionManifestUrl
        {
            get
            {
                ...
                {
                    Uri uri2 = new Uri(baseUri, "/updates/versions.7.test.txt");
                    return uri2.ToString();
                }
                string str2 = 7.ToString(CultureInfo.InvariantCulture);
                ...
                string neutralLocaleName = GetNeutralLocaleName(PdnResources.Culture);
