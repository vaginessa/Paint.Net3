    ...
    internal class DownloadingState : UpdatesState, INewVersionInfo
    {
        ...
        public override void OnEnteredState()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (handler2 == null)
                    {
                        handler2 = (sender, e) => base.OnProgress(e.Percent);
                    }
                    ProgressEventHandler progressCallback = handler2;
                    Utility.DownloadFile(new Uri(this.downloadMe.ChooseDownloadUrl(flag)), output, progressCallback);
                    output.Flush();
        ...
        public override void ProcessInput(object input, out PaintDotNet.State newState)
        {
            ...
            else if (input.Equals(PrivateInput.GoToError))
            {
                ...
                {
                    str = PdnResources.GetString2("Updates.DownloadingState.GenericError");
                }
